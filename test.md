...
- 그러나 비동기 처리는 동시에 일을 처리함

- 구조적으로 자바스크립트는 싱글 스레드인데 비동기 처리를 할 수 없음

- 그러나 **자바스크립트 런타임(실행환경)으로 인하여 자바스크립트는 싱글 스레드지만 비동기처리가 가능 하게 됨**

- 자바스크립트 런타임에 구성요소로 V8엔진(콜 스택(Call Stack)과 메모리 힙(Memory Heap)), 콜백 큐 또는 테스크 큐 (Callback Queue 또는 Task Queue) 및 이벤트 루프 (Event Loop), 웹 API 로 구성 됨
